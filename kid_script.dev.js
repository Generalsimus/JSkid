...
(function () {
        ...
        function run_body_code() {
          ...
          if (responsWS.data.length) {
            ...
            function create_code(element) {
              ...
              // console.log(element.line_coll, element.path, element.code.split(/\r?\n/).slice(line_coll[0] - 3, line_coll[0] + 3).join(/\r/))
              // switch (element.path.split(".").pop().toupe) {

              // }
              // console.log(line_coll, Math.abs(Math.min(line_coll[0] - 6,0) - 5)  )
                  ...
                  pre: [
                    {
                      div: LINE.map(function (v, i) {
                        return { div: Math.max(line_coll[0] - 5, 0) + i + 1 }
                      }),
                      style: "border-right: 1px solid #642b34;margin-right: 20px;display: flex;flex-direction: column;padding-right: 1em;"
                    },
                    {
                      // line_coll[0]+line_coll[0] - 5
                      div: Array.from(Array(Math.abs(Math.min(line_coll[0] - 6, 0) - 5)), function (v, i) {
                        return { div: " ", style: "width: 100%;background: #48242e;background: rgb(227 48 48 / " + ((i + 1) == line_coll[0] - Math.max(line_coll[0] - 5, 0) ? 0.2 : 0) + ");" }
                      }),
                      // [
                      //   {div:" ",style:"width: 100%;background: #48242e;background: rgb(227 48 48 / 0.2);"}
                      // ],
            ...
            KD_T(ERROR_NODE_FOR_KID, {
              div: [
                { h2: responsWS.general_error, style: "color: #e33030;" },
                responsWS.data.reverse().map(function (element) {
                  // console.log(element)
                  return element.code ? [
                    { div: element.error_cod, style: "color: #59769b;word-break: break-word;margin: 1vw 0;" },
  ...
  // window.onerror = function(message, source, lineno, colno, error) { console.log(arguments) };
  window.addEventListener('error', function (event) {
    // console.log(event)
    // console.error(new Error("TypeError: Assignment to constant variable.↵    at HTMLDivElement.click (http://localhost:27405/kix/APP.js:4:21)"))

    // KD_T(document.body, { div: [event.message, event.error] })
    // KD_T(document.body, <li key={e.message}><RedBox error={e.error} />dfdfd</li>)
    ...
    var error_list = event.error.stack.split(/\r?\n/),
      error_data = flat(error_list.slice(1).map(function (error_cod) {
        ...
        if (url_match && url_match.length) {
          ...
          return { path: path, line_coll, error_cod: error_cod.trim() }
        } else {
          return []
        }
      })),
    ...
    error_data = error_data.findIndex(function (v) { return v.path == error_path }) < 0 ? [
      { path: new URL(event.filename).pathname, line_coll: [event.lineno, event.colno], error_cod: event.message }
    ].concat(error_data) : error_data;

    if (socket.readyState === WebSocket.OPEN) {
      socket.send(JSON.stringify({ method: "get_error_files", data: error_data, general_error: event.message }))
    } else {
